    ...
    if os.path.exists(SESSION_FILE):
        try:
            with open(SESSION_FILE, "r") as f:
                ...
                st.session_state["last_active"] = data.get("last_active")
...
if st.session_state.get("last_active"):
    try:
        last_active = datetime.strptime(st.session_state["last_active"], "%Y-%m-%d %H:%M:%S")
...
st.session_state["last_active"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
if st.session_state.logged_in:
    ...
    if choice == "Register":
        ...
        if st.button("Register", use_container_width=True, key="register_btn"):
            ...
            elif "@gmail.com" not in email or not email.endswith(".com"):
    ...
    # ---------------- LOGIN ----------------
        ...
        if st.session_state.get("show_reset", False):
            ...
            confirm_pass = st.text_input("Confirm New Password", type="password", key="confirm_pass_input")
            ...
            if st.button("Change Password", use_container_width=True, key="change_pass_btn"):
                if not reset_email or not new_pass or not confirm_pass:
                    ...
                    success, msg = reset_password(reset_email, new_pass)
                    if success:
