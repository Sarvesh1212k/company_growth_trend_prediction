...
def save_session():
    ...
    session_data = {
        # ❌ do not save logged_in or username (per-device login only)
    ...
    if os.path.exists(SESSION_FILE):
        try:
            with open(SESSION_FILE, "r") as f:
                data = json.load(f)
                # ❌ do not restore logged_in or username
...
# ---------------- Initialize Defaults ----------------
defaults = {
    "logged_in": False,   # now only temporary per device
    "username": None,     # same
    "show_reset": False,
    "reset_email": "",
        ...
        if st.button("Login", use_container_width=True, key="login_btn"):
            if not user_input or not password:
                ...
                if user and user != "not_registered":
                    ...
                    st.rerun()  # ensure navigation after login
